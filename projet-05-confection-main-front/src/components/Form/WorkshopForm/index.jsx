...
const WorkshopForm = () => {
  ...
  useEffect(() => {
    // Récupérer l'ID de l'utilisateur depuis "location" userId étant stocké depuis la page FormInscription
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      isNaN(parseInt(formData.registration_number)) ||
      formData.description.length < 80 ||
      formData.zipcode.length !== 5 ||
      isNaN(parseInt(formData.zipcode)) ||
      formData.phone_number.length !== 10 ||
      isNaN(parseInt(formData.phone_number)) ||
      formData.picture === ""
    ) {
