...
function CompagnyInformation() {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [userId]); // Le tableau vide en second argument signifie que cet effet ne s'exécute qu'une fois après le montage du composant
...
const handleSubmit = async (e) => {
  ...
  try {
   ...
   return (
    <div style={{ minWidth: "350px", width: "100%", padding: "0 20px" }}>
      <Form onSubmit={handleSubmit} style={{ fontSize: '1.1em' }}>
        ...
        <Form.Group widths="equal">
          ...
          <Form.Field>
            <label style={{ fontWeight: 'bold' }}>Nom entreprise</label>
            <Input
              type="text"
              placeholder="Nom entreprise"
              value={companyData.workshop_name}
              onChange={(e) =>
                setCompanyData({ ...companyData, workshop_name: e.target.value })
              }
              disabled={!editMode}
            />
          </Form.Field>
          <Form.Field>
            <label style={{ fontWeight: 'bold' }}>Numéro SIRET</label>
            <Input
              type="text"
          ...
          <Form.Field>
            ...
            <Input
              ...
              onChange={(e) =>
                setCompanyData({ ...companyData, workshop_phone_number: e.target.value })
              }
              disabled={!editMode}
            />
          </Form.Field>
        </Form.Group>
        {/* Champ pour l'workshop_email professionnel */}
        <div>
          <h2>Adresse E-mail Pro</h2>
          <Form.Field>
            <label style={{ fontWeight: 'bold' }}>E-mail Pro</label>
            <Input
              type="email"
              placeholder="E-mail Pro"
              value={companyData.workshop_email}
              onChange={(e) =>
                setCompanyData({ ...companyData, workshop_email: e.target.value })
              }
              disabled={!editMode}
            />
          </Form.Field>
        ...
        <div>
          ...
          <Form.Field>
            ...
            <Input
              ...
              onChange={(e) =>
                setCompanyData({ ...companyData, workshop_description: e.target.value })
              }
              disabled={!editMode}
            />
          </Form.Field>
          ...
          {companyData.socialMedia &&
            companyData.socialMedia.map((social, index) => (
              <Form.Group key={index} widths="equal">
                <Form.Field>
                  <Dropdown
                    ...
                    onChange={(e, { value }) =>
                      setCompanyData({
                        ...companyData,
                        socialMedia: companyData.socialMedia.map((item, i) =>
                ...
                {editMode && (
                  <Button icon onClick={() => handleRemoveSocialMedia(index)}>
                    <Icon name="trash" />
                  </Button>
                )}
              </Form.Group>
